...
class Studiewijzers extends MagisterApi {
  ...
  Future studiewijzer() async {
    account.studiewijzers = (await loadWijzer())["Items"].map((wijs) => Wijzer(wijs)).toList().cast<Wijzer>();
  }
  ...
  Future loadChildren(Wijzer wijs) async {
    ...
    wijs.children = wijzer["Onderdelen"]["Items"].map((wijs) => Wijzer(wijs)).toList().cast<Wijzer>() as List<Wijzer>;
    account.save();
  }

  Future loadTab(Wijzer wijs) async {
    Wijzer wijzer = Wijzer((await api.dio.get(wijs.tabUrl)).data);
